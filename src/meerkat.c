...
static our_mutex_t return_table_mutex;

//Variable qui servira à donner un id à chaque thread
static int global_id = 0;
static our_mutex_t global_id_mutex;
//
...
int thread_init(void)
{
	...
	our_mutex__init(&global_id_mutex);
	our_mutex__init(&thread_count_mutex);
	our_mutex__init(&runqueue_mutex);
	our_mutex__init(&join_queue_mutex);
	our_mutex__init(&return_table_mutex);
...
int thread_create(thread_t * newthread, void *(*start_routine) (void *), void *arg)
{
	...
	our_mutex__lock(global_id_mutex);
	new_thread->id = global_id++;
	our_mutex__unlock(global_id_mutex);
	...
	our_mutex__lock(thread_count_mutex);
	++thread_count;
	our_mutex__unlock(thread_count_mutex);
...
int thread_join(thread_t thread, void **retval)
{
	...
	if(!found)
	{
		...
		{
			our_mutex__lock(return_table_mutex);
			*retval = htable__find_int(return_table, thread);
			...
			our_mutex__unlock(return_table_mutex);
		}
		return 0;
	}
...
void free_ressources(void)
{
	...
	our_mutex__destroy(&global_id_mutex);
	our_mutex__destroy(&thread_count_mutex);
	our_mutex__destroy(&runqueue_mutex);
	our_mutex__destroy(&join_queue_mutex);
	our_mutex__destroy(&return_table_mutex);
